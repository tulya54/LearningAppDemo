...
import com.thirtyeight.thirtyeight.presentation.ui.views.mechanics.wordsearch.Vec2.Companion.normalize
import com.thirtyeight.thirtyeight.presentation.ui.views.mechanics.wordsearch.Vec2.Companion.sub
import java.util.*
...
class StreakView @JvmOverloads constructor(
    ...
    init {
        ...
        attrs?.let {
            ...
            streakLineWidth =
                    a.getDimensionPixelSize(R.styleable.StreakView_streakWidth, streakLineWidth)
    ...
    override fun onAttachedToWindow() {
        ...
        if (gridId != -1 && snapToGridType != SnapType.NONE) {
            grid = rootView.findViewById<View>(gridId) as BaseMatrixItem
        }
    }
    ...
    override fun onDraw(canvas: Canvas) {
        for (line in lines) {
            ...
            rect[line.start.x - halfWidth, line.start.y - halfWidth, line.start.x + length + halfWidth] =
                    line.start.y + halfWidth
            canvas.drawRoundRect(rect, halfWidth.toFloat() / 2, halfWidth.toFloat() / 2, paint)
            canvas.restore()
        }
    }
    ...
    class StreakLine {
        var start: Vec2 = Vec2()
        var end: Vec2 = Vec2()
        var startIndex: Coordinate = Coordinate(-1, -1)
        var endIndex: Coordinate = Coordinate(-1, -1)
        var color = Color.RED
    }
    ...
    private fun Coordinate.set(row: Int, col: Int) {
        this.row = row
        this.col = col
    }
}